    ...
    while True:
        ...
        if not success:
            ...
            while retry_count < 3 and not success:
                ...
                retry_count += 1
                print(
                    f"[DEBUG] Retry {retry_count}: Frame read {'successful' if success else 'failed'}.)"
                )

            if not success:
                print(
                    "[ERROR] Unable to read frame after multiple retries. Check camera connection."
                )
                break
        ...
        if exercise_running and exercise_engine.exercise:
            ...
            if in_rest_period:
                ...
                if elapsed_rest >= REST_DURATION:
                    ...
                    draw_text_with_background(
                        frame,
                        f"Set {sets_completed} done  |  Next: Set {sets_completed + 1}",
                ...
                results = pose_estimator.estimate_pose(
                    frame, exercise_engine.exercise_name
                )

                if results.pose_landmarks:
                    # NEW: Use Exercise Engine to process frame
                    result = exercise_engine.process_frame(
                        frame, results.pose_landmarks.landmark
                    )

                    if (
                        not result["success"]
                        and result.get("error") == "Key landmarks not visible"
                    ):
                        draw_text_with_background(
                            frame,
                        ...
                        not result["success"] and result.get("error") == "Getting ready"
                    ):
                        secs = result.get("grace_remaining", 0)
                        draw_text_with_background(
                            frame,
                            f"Get ready... {secs:.1f}s",
                    ...
                    if result["success"]:
                        # Draw status overlay
                        exercise_engine.draw_status_overlay(
                            frame, exercise_goal, sets_goal, sets_completed
                        )
                        ...
                        if current_counter >= exercise_goal:
                            ...
                            if sets_completed >= sets_goal:
                                ...
                                draw_text_with_background(
                                    frame,
                                    f"WORKOUT COMPLETE! Avg Score: {avg_score}",
            ...
            cv2.putText(
                frame,
                f"Available: {len(exercises)} exercises",
                (frame.shape[1] // 2 - 120, frame.shape[0] // 2 + 40),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (200, 200, 200),
                1,
...
def dashboard():
    ...
    try:
        ...
        weekly_workout_count = sum(
            day["workout_count"] for day in weekly_stats.values()
        )
...
def start_exercise():
    ...
    if exercise_type not in available:
        return jsonify(
            {
                "success": False,
                "error": f"Invalid exercise type. Available: {available}",
            }
        )
    ...
    if not exercise_engine.set_exercise(exercise_type):
        return jsonify(
            {"success": False, "error": f"Failed to load exercise: {exercise_type}"}
        )
    ...
    logger.info(
        f"Started exercise: {exercise_type}, goal: {exercise_goal} reps x {sets_goal} sets"
    )
...
def get_status():
    ...
    status = {
        ...
        "rest_remaining": (
            max(0, REST_DURATION - int(time.time() - rest_start_time))
            if in_rest_period and rest_start_time
            else 0
        ),
    }
...
def list_exercises():
    ...
    return jsonify(
        {"exercises": exercises, "info": exercises_info, "count": len(exercises)}
    )
...
def profile():
    ...
    return render_template(
        "profile.html", user=user, stats=stats, favorites=favorites, settings=settings
    )
...
def process_video_subprocess(video_id):
    ...
    try:
        ...
        if process.returncode == 0 and os.path.exists(output_json_path):
            ...
            logger.info(
                f"Video processing completed: {analysis['reps']} reps, output: {output_video_path}"
            )
        else:
            analysis["status"] = "error"
...
def get_video_status(video_id):
    ...
    if analysis.get("processed_video") and os.path.exists(
    ...
    return jsonify(
        {
            ...
            "processed_video_url": (
                f"/api/video/processed/{video_id}" if has_processed_video else None
            ),
        }
    )
